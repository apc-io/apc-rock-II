...
extern int read_ubi_nand(unsigned int addr, unsigned int len, size_t * retlen, u_char * buf);
extern int write_ubi_nand(unsigned int addr, unsigned int len, size_t * retlen, u_char * buf);
extern int erase_ubi_nand(unsigned int block);
		...
		int len)
{
	...
	if (err){
		...
		printk("\nerr is happened in block %d", pnum);
...
retry:
	err = read_ubi_nand(addr, len, &read, buf);
		 ...
		 int len)
{
	...
	addr = (loff_t)pnum * ubi->peb_size + offset;
	err = write_ubi_nand(addr, len, &written, buf);
	...
	if (magic != UBI_EC_HDR_MAGIC) {
		...
		if (read_err != -EBADMSG &&
			...
			err = paranoid_check_all_ff(ubi, pnum, 0,
						    ubi->peb_size);
			//print_nand_buf((unsigned char *)ec_hdr, UBI_EC_HDR_SIZE);
			if (err)
				 ...
				 int len)
{
	...
	len = ubi->min_io_size * 2;
	mutex_lock(&ubi->dbg_buf_mutex);
	err = read_ubi_nand(addr, len, &read, ubi->dbg_peb_buf); 
