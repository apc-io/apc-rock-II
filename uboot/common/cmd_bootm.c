...
#if 0
		/* don't check crc32 for reducing boot time */
...
# error Unknown CPU type
	...
	case IH_COMP_NONE:
		if(ntohl(hdr->ih_load) == addr) {
			...
			int num = 0;
			ulong copy_size;
			ulong src_tmp, des_tmp;
			copy_size = (ntohl(hdr->ih_load) - data); /* get copy size without overlay areas */
			src_tmp = data + len; /*assign source address. */
			des_tmp = (ntohl(hdr->ih_load) + len); /* assign destination address */
			/* printf("src_tmp = %x, des_tmp = %x, move_size = %x\n",
						src_tmp, des_tmp, copy_size); */

			/* destination address between source address and source+datasize */
			if ((ntohl(hdr->ih_load) >= data) && (ntohl(hdr->ih_load) <= (data+len))) {
				src_tmp -= copy_size;
				des_tmp -= copy_size;
				while (len >= copy_size) {
					/* printf("%d, src_tmp = %x, des_tmp = %x, len = %x\n",
								num, src_tmp, des_tmp, len); */
					memcpy((void *) des_tmp, (uchar *) src_tmp, copy_size);
					len -= copy_size;
					src_tmp -= copy_size;
					des_tmp -= copy_size;
					num++;
					...
					copy_size = len;
					/* printf("src_tmp = %x, des_tmp = %x, len = %x\n",
							src_tmp, des_tmp, copy_size); */
					memcpy((void *) ntohl(hdr->ih_load), (uchar *) data, copy_size);
			} else {
