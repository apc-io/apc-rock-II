...
static int device_validate(u8 type, u8 num, u32 *size)
{
	if (num < CFG_MAX_NAND_DEVICE) {
		//extern struct nand_chip[0];
		extern struct nand_chip nand_dev_desc[CFG_MAX_NAND_DEVICE];
		*size = nand_dev_desc[0].totlen;
		return 0;
	}
...
struct part_info* ubi_part_info(struct mtd_device *dev, unsigned int part_num)
{
	struct list_head *entry;
	struct part_info *part;
	int num;
		...
		u8 *part_num, struct part_info **part)
{
	...
	*dev = current_dev;
	if ((*part = ubi_part_info(*dev, pnum)) == NULL) {
...
static int do_ubi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strcmp(argv[1], "part") == 0) {
		...
		if (argc == 2) {
			...
			printf("Device %d:, partition %s\n",
			       ubi_dev.nr, ubi_dev.part_name);
			return 0;
		}
		...
		int start = (int)simple_strtoul(argv[3], NULL, 16);
    		int size = (int)simple_strtoul(argv[4], NULL, 16);
