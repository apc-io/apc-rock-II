...
typedef struct {
    ...
    unsigned int color_key_mode : 3; // (Non-Hit,Hit):0/1-(alpha,alpha),2-(alpha,pix1),
									 // 3-(pix1,alpha),4-(alpha,pix2),5-(pix2,alpha),6-(pix1,pix2),7-(pix2,pix1)
...
#define VPPIO_SCLSET_OVERLAP        _IOW(VPPIO_MAGIC,VPPIO_SCL_BASE+4,vpp_overlap_t)
