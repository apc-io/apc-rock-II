...
#ifdef __KERNEL__
	if (!g_vpp.hdmi_certify_flag) {
		schedule_delayed_work(&vo_hdmi_plug_work,
			msecs_to_jiffies(HDMI_PLUG_DELAY));
	} else
#endif
	vo_hdmi_do_plug(0);
	return IRQ_HANDLED;
...
static int vo_hdmi_config(vout_t *vo, int arg)
{
	...
	hdmi_info.option = vout_get_edid_option(VPP_VOUT_NUM_HDMI);
...
int vout_check_display_info(struct vout_init_parm_t *init_parm)
{
	...
	for (i = 0; i < 2; i++) {
		...
		case VOUT_LVDS:
			...
			/* --> added by howayhuo to set vmode according to wmt.display.param
				It fix the bug that the 768 x 1024 LVDS can't normally show logo */
			vmode->xres = parm[3];
			vmode->yres = parm[4];
			vmode->refresh = parm[5];
			/* <-- end add */
