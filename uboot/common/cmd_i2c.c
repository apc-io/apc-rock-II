...
int do_i2c_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	do {
		...
		if(i2c3_read(chip, addr, alen, linebuf, linebytes) != 0) {
...
int do_i2c_mw ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	while (count-- > 0) {
		if(i2c3_write(chip, addr++, alen, &byte, 1) != 0) {
...
int do_i2c_reg_set(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if(i2c3_write(chip, addr, alen, (uchar *)mem_addr, count) != 0)
		puts ("Error writing the chip.\n");
...
int do_i2c_crc (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	err = 0;
	while(count-- > 0) {
		if(i2c3_read(chip, addr, alen, &byte, 1) != 0) {
			err++;
		}
...
mod_i2c_mem(cmd_tbl_t *cmdtp, int incrflag, int flag, int argc, char *argv[])
{
	...
	do {
		printf("%08lx:", addr);
		if(i2c3_read(chip, addr, alen, (uchar *)&data, size) != 0) {
		...
		if (nbytes == 0) {
			...
			if (nbytes) {
				...
				reset_cmd_timeout();
#endif
				if(i2c3_write(chip, addr, alen, (uchar *)&data, size) != 0) {
					puts ("Error writing the chip.\n");
				}
...
int do_i2c_loop(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	 ...
	 */
	while(1) {
		if(i2c3_read(chip, addr, alen, bytes, length) != 0) {
			puts ("Error reading the chip.\n");
		}
		udelay(delay);
	}
...
int do_sdram  ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if(i2c3_read(chip, 0, 1, data, sizeof(data)) != 0) {
