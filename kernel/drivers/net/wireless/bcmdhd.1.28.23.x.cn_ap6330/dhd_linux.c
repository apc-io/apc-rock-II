...
void dhd_enable_packet_filter(int value, dhd_pub_t *dhd)
{
	...
	if (dhd_pkt_filter_enable && (!value ||
		...
		for (i = 0; i < dhd->pktfilter_count; i++) {
#if defined(PASS_ARP_PACKET) && !defined(FILTER_OUT_ALL_PACKETS)
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	printk("set wifi name to bcmdhd_ap6330.ko\n");
	set_wifi_name("bcmdhd_ap6330.ko");
...
dhd_module_init(void)
{
	...
	pwm_32KHZ_control(1);
	              //add by dulong.
    error = gpio_request(WMT_PIN_GP62_SUSGPIO1, "wifi_chip_en");
    if(error < 0) {
