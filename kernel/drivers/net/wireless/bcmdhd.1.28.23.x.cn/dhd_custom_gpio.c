...
void bcm_wlan_power_off(int mode){
    if(mode == 1){
        force_remove_sdio2(); 
        wifi_power_ctrl(0);
	    gpio_free(wmt_intr_gpio);
    }else if(mode == 2)
        wifi_power_ctrl(0);
    else
        printk("xxxxxxxxxxxxxxxxxxxxxxxxxxx%s %d\n",__func__,__LINE__);
}
void bcm_wlan_power_on(int mode)
{
    if(mode == 1){
    	if (gpio_request(wmt_intr_gpio, "mtk6620_intr") < 0) {
    		printk("gpio(%d) mtk6620 intr gpio request fail\n", wmt_intr_gpio);
    	}	
        wifi_power_ctrl(1);
        wmt_detect_sdio2();
    }else if(mode == 2)
...
#if defined  CUSTOMER_WMT
    host_oob_irq = IRQ_GPIO;
#endif
