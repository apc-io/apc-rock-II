...
#ifdef HW_OOB
	irq_flags =
		IORESOURCE_IRQ | IORESOURCE_IRQ_HIGHLEVEL | IORESOURCE_IRQ_SHAREABLE|IRQF_SHARED;
#else
...
//	if (curstate != enable) {
	if (1) {
		if (enable){
            ...
            wmt_gpio_ack_irq(wmt_intr_gpio);
            wmt_gpio_unmask_irq(wmt_intr_gpio);
		}else{
			//disable_irq_nosync(sdhcinfo->oob_irq);
			wmt_gpio_mask_irq(wmt_intr_gpio);
			wmt_gpio_ack_irq(wmt_intr_gpio);
...
#if 1
	if(!is_gpio_irqenable(wmt_intr_gpio) || !gpio_irqstatus(wmt_intr_gpio))
			return IRQ_NONE;			
...
int bcmsdh_register_oob_intr(void * dhdp)
{
	...
	if (!sdhcinfo->oob_irq_registered) {
		...
		/*enable pull down*/
        gpio_direction_input(wmt_intr_gpio);
		SDLX_MSG(("%s: IRQ=%d Type=%X \n", __FUNCTION__,
			(int)sdhcinfo->oob_irq, (int)sdhcinfo->oob_flags));
		/* Refer to customer Host IRQ docs about proper irqflags definition */
		error = request_irq(sdhcinfo->oob_irq, wlan_oob_irq, sdhcinfo->oob_flags,
			"bcmsdh_sdmmc", (void*)&aaaa);
		if (error){
            printk("request irq error %d\n",error);
			return -ENODEV;

        }
		wmt_gpio_set_irq_type(wmt_intr_gpio,IRQ_TYPE_LEVEL_HIGH);
#if defined(CONFIG_ARCH_RHEA) || defined(CONFIG_ARCH_CAPRI)
		if (device_may_wakeup(sdhcinfo->dev)) {
#endif
            printk("%s %d delete code \n",__func__,__LINE__);
...
void bcmsdh_unregister_oob_intr(void)
{
	...
	if (sdhcinfo->oob_irq_registered == TRUE) {
		...
		free_irq(sdhcinfo->oob_irq, (void*)&aaaa);
