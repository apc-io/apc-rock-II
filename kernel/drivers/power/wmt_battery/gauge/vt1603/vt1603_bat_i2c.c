...
static void wm8650_wakeup_init(int wk_src)
{
    ...
    REG8_VAL(GPIO_CTRL_GP62_WAKEUP_SUS_BYTE_ADDR) &= ~wk_src;
    /* disable WAKE_UP output */
    REG8_VAL(GPIO_OC_GP62_WAKEUP_SUS_BYTE_ADDR) &= ~wk_src;
    /* enable pull high/low   */
    REG8_VAL(PULL_EN_GP62_WAKEUP_SUS_BYTE_ADDR) |= wk_src;
    /* set pull high WAKE_UP  */
    REG8_VAL(PULL_CTRL_GP62_WAKEUP_SUS_BYTE_ADDR) |= wk_src;
    /* enable WAKE_UP last    */
    REG8_VAL(GPIO_CTRL_GP62_WAKEUP_SUS_BYTE_ADDR) |= wk_src;
...
static int __init vt1603_bat_uboot_env_check(void)
{
	...
	* 1:vt1603:1000:b78:972:b29:b11:afe:aca:aab:a94:a80:a64:a40:a1b:972 */
	...
	if (strncmp(tmp_buf,"vt1603",6)) {
		if (strncmp(tmp_buf,"vt1603k",7)) {
			bat_dbg("VT1603 Battery disabled now\n");
			ret = -ENODEV;
			goto out;
		}
	...
	sscanf((buf + i), "%d", &bat_update_interval);
