...
static int power_reset = 14;
static int power_en = 15;
...
int initpower(void){
    int result = 0;
#if defined(CONFIG_SMS_SDIO_DRV)
	power_en = WMT_PIN_GP1_GPIO15;
	power_reset = WMT_PIN_GP1_GPIO14;
#elif defined(CONFIG_SMS_USB_DRV)
	power_en = WMT_PIN_GP62_WAKEUP2;
	power_reset = WMT_PIN_GP62_WAKEUP3;
#endif

#if defined(CONFIG_SMS_USB_DRV) || defined(CONFIG_SMS_USB_DRV)
	result = gpio_request(power_en, "isdbt-power-en");
	if (result < 0) {
	...
	result = gpio_request(power_reset, "isdbt-power-reset");
	if (result < 0) {
			printk("gpio(%d) request fail\n", power_reset);
	  		//kfree(smspwr_devp);
                         gpio_free(power_en);
	...
	gpio_direction_output(power_reset, 0);
	gpio_direction_output(power_en, 0);
	printk("output GPIO %d %d requested successfully.\n", power_en, power_reset);
#endif
	return 0;

}
void sms_poweron(void)
{
#if defined(CONFIG_SMS_SDIO_DRV) || defined(CONFIG_SMS_USB_DRV)
	printk("sms_poweron\n");
	gpio_set_value(power_en, 1);
	msleep(20);
	gpio_set_value(power_reset, 1);
#endif
}

void sms_poweroff(void)
{
	printk("sms_poweroff\n");
#if defined(CONFIG_SMS_SDIO_DRV) || defined(CONFIG_SMS_USB_DRV)
	gpio_set_value(power_reset, 0);
...
void smschar_unregister(void)
{
	...
	force_remove_sdio2();
#endif
#if defined(CONFIG_SMS_USB_DRV) || defined(CONFIG_SMS_SDIO_DRV)
	msleep(300);
	sms_poweroff();
    gpio_free(power_en);
    gpio_free(power_reset);
#endif
