...
#define G_2_REVERSE	0 //!!!!! 1
#define GSE_LOG(fmt, args...)  //printk
...
static  int mc32x0_i2c_write(unsigned char reg_addr, unsigned char *data, unsigned char len)
{
	...
	int ret;
	ret = i2c_api_do_send(0, MC32X0_I2C_ADDR, reg_addr, data, len);
	if (ret <= 0) {
...
static int mc32x0_i2c_read(unsigned char reg_addr, unsigned char *data, unsigned char len) 
{
	...
	int ret;
	ret = i2c_api_do_recv(0, MC32X0_I2C_ADDR, reg_addr, data, len);
	if (ret <= 0) {
...
if(gain_data[0] == 0)
{
	...
	return 0;
		...
		//buffer[2]= 127*3;//test 2013-10-23
		#ifdef SUPPORT_VIRTUAL_Z_SENSOR  // add 2013-10-23
		if (g_virtual_z)
		{
			tempX = buffer[MC32X0_AXIS_X];
...
static long mc32x0_ioctl(/*struct inode *inode,*/ struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	// report data
		...
		rxyz[1] = acc[1];
		rxyz[2] = acc[2];
		tbuf[0] = rxyz[l_sensorconfig.xyz_axis[0][0]]*l_sensorconfig.xyz_axis[0][1] + l_sensorconfig.offset[0];
		tbuf[1] = rxyz[l_sensorconfig.xyz_axis[1][0]]*l_sensorconfig.xyz_axis[1][1] + l_sensorconfig.offset[1];
		tbuf[2] = rxyz[l_sensorconfig.xyz_axis[2][0]]*l_sensorconfig.xyz_axis[2][1] + l_sensorconfig.offset[2];
		for (i=0; i<3; i++){
			if (tbuf[i]&0x80)
			{
				tbuf[i] -= 256;
		...
		if(copy_to_user((unsigned char*)arg, tbuf, 3)!=0)
		{
			
			err = -EFAULT;
		}
		break; //add for mesicd to get data 2013-7-18
...
static int get_axisset(void* param)
{
	...
	if (wmt_getsyspara("wmt.io.mc3230sensor", varbuf, &varlen)) {
		...
		l_sensorconfig.offset[0] = (s16)tmpoff[0];
		l_sensorconfig.offset[1] = (s16)tmpoff[1];
		l_sensorconfig.offset[2] = (s16)tmpoff[2];
