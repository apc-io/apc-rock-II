...
static int reset_afeter_bl_on = 0;
...
//RECHECK:	
	...
	#endif	
		printk(" %s gsensor I2C err @@@ REG_STADR_VALUE = %d , REG_STAINT_VALUE = %d \n", 
...
{
	flush_delayed_work_sync(&devdata.delaywork);
	cancel_delayed_work_sync(&devdata.delaywork);
...
Retry:	
	if (this_client){
		ret = gsensor_reset(this_client);
	}	
	if (ret < 0) {
		retry++;
		printk("%s gsensor_reset error! retry %d\n", __FUNCTION__, retry);
		if (retry < 10){
			msleep(1);
			goto Retry;
		}	
		else {
			reset_afeter_bl_on = 1;
			return ret;
		}	
	}	
	...
	void *dummy)
{
	...
	switch (event) {
		...
		case BL_OPEN:
			...
			//printk("\nopen backlight\n\n");
			//printk("enable irq\n\n");
			if (reset_afeter_bl_on) {
				printk("<<<%s %s bl open!\n", __FILE__, __FUNCTION__);
				gsensor_reset(this_client);
			}
			break;
	}
...
static struct notifier_block wmt_d10_notify = {
...
static void __exit device_exit(void)
{
	...
	unregister_bl_notifier(&wmt_d10_notify);
