...
int get_gsensor_conf(struct gsensor_conf *gs_conf)
{
	...
	for (i = 0; i < 10; i++) {
		...
		gs_conf->name[i] = varbuf[i+2];
	}
	if (i == 10) {
		printk("wmt.io.gsensor's name size exceed 10!\n");
		return -1;
	}

	/* Check direction */
	n = sscanf(varbuf+i+3, "%d:%d:%d:%d:%d:%d",
	...
	printk(KERN_DEBUG "wmt.io.gsensor = %d:%s:%d:%d:%d:%d:%d:%d\n",
	gs_conf->op,
	gs_conf->name,
	gs_conf->xyz_axis[0][0],
	gs_conf->xyz_axis[0][1],
