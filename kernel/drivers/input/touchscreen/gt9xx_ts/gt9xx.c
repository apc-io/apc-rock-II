...
int wmt_ts_load_firmware(char* firmwarename, unsigned char** firmdata, int* fwlen)
{
	...
	if(request_firmware(&fw_entry, firmwarename, &i2c_connect_client->dev)!=0) {
		GTP_ERROR(KERN_ERR "cat't request firmware.");
		return -1;
	}
	if (fw_entry->size <= 0) {
		GTP_ERROR(KERN_ERR "load firmware error.");
...
int read_cfg(char* cfgname, u8* cfg, int len_max)
{
	...
	if (wmt_ts_load_firmware(cfgname, &rawdata, &len))
		return -1;
