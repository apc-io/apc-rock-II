...
static int rmtctl_probe(struct platform_device *dev)
{
	...
	if (wmt_getsyspara("wmt.cir.param", buf, &varlen) == 0) {
		//printk("wmt.cir.param = %s\n", buf);
		if (memcmp(buf, "1", 1) == 0)
			printk("rmtctl enable!\n");
		else if (memcmp(buf, "0", 1) == 0){
			printk("rmtctl disable!\n");
			return 0;
		}
	}
	...
	/* Register an ISR */
	i = request_irq(IRQ_CIR, rmtctl_interrupt, IRQF_SHARED, "rmtctl", idev);
...
MODULE_DESCRIPTION("WMT Remote driver");
