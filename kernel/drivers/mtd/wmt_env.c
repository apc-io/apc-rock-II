...
#define SD_UBOOT_ENV1 "u-boot env. cfg. 1-SD"
#define SD_UBOOT_ENV2 "u-boot env. cfg. 2-SD"
#define USB_UBOOT_ENV1 "u-boot env. cfg. 1-USB"
#define USB_UBOOT_ENV2 "u-boot env. cfg. 2-USB"
...
static unsigned int env_crc32(unsigned char	*buf)
{
	...
	val = STRAP_STATUS_VAL;
	if ((val&0x4008) != 0x4000)
		len = ENV_MAX_SIZE - 4;
...
int read_env_before_mtd_init(int uenv[2], int bootflash, unsigned int offset[2])
{
	...
	env = uenv[0];
	if ((!mtd_table[0]) && (env != ENV_INVALID) && (bootflash == SPI_FLASH_TYPE)) {
		#ifdef CONFIG_MTD_WMT_SF
...
int read_env_after_mtd_init(void)
{
	...
	if (env_valid == 0xFF && env_invalid != 0xFF) {
		if (mtd_table[env_invalid]->_read(
...
static unsigned int env_init(void)
{
	...
	unsigned int val, crc32, env, blk_cnt = 0;
	int bootflash = SPI_FLASH_TYPE;
	unsigned int env1 = ENV_INVALID, env2 = ENV_INVALID, offs = 0;
	char *boot_str1, *boot_str2;
	//unsigned int chip_id = *(volatile unsigned long *)(SYSTEM_CFG_CTRL_BASE_ADDR) >> 16;
	val = STRAP_STATUS_VAL;
...
next_blk:
		while (offs < (NAND_ENV_BLK_LENGTH * mtd_table[0]->erasesize)) {
			...
			if (!(mtd_table[0]->_block_isbad(mtd_table[0], offs)))
				break;
			else {
				blk_cnt++;
	...
	} else if (bootflash == SPI_FLASH_TYPE) {
		if (!mtd_table[0])
...
//printk(KERN_WARNING "update 1 mtd_init=%d\n", mtd_init);
	...
	if (bootflash == NAND_FLASH_TYPE) {
		...
		while (chk_offs < mtd_table[env_valid]->size && i < blocks_cnt) {
			if (mtd_table[env_valid]->_block_isbad(mtd_table[env_valid], offs)) {
	...
	if ((bootflash == NAND_FLASH_TYPE)) {
		for (i = 0; i < blocks_cnt; i++) {
			...
			if (mtd_table[env_valid]->_erase(mtd_table[env_valid], &ei) != 0) {
...
//printk(KERN_WARNING "update 5\n");
	...
	} else	/*sfboot after mtd is init*/
		if (mtd_table[env_invalid]->_erase(mtd_table[env_invalid], &ei) != 0) {
	...
	#endif
	{
		...
		if (mtd_table[env_invalid]->_write(
...
int wmt_getsyspara(char *varname, unsigned char *varval, int *varlen)
{
	...
	if (env_ptr == NULL || (unsigned int)env_ptr != (unsigned int)env_ptr_nand || (!mtd_init)) {
		...
		mtd = get_mtd_device_nm(boot_str1);
		if ((envInit != 0x55AA && IS_ERR(mtd)) ||
		(envInit == 0x55AA && !(IS_ERR(mtd)))) {
			if (env_init()) {
				rcode++;
				goto out;
			}
...
int wmt_getsocinfo(unsigned int *chipid, unsigned int *bondingid)
{
	...
	*bondingid = BONDING_OPTION_4BYTE_VAL;
