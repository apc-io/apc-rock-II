...
static int wmt_pcm_controller_init(void)
{
	...
	wmt_pcm_controller.pcm_clk_src = auto_pll_divisor(DEV_PCM1, GET_FREQ, 0, 0);
...
static int wmt_pcm_controller_dai_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	printk(KERN_INFO "wmt_pcm_controller_dai_prepare byte = %d, channels = %d", byte, runtime->channels);
...
static int wmt_pcm_controller_probe(struct platform_device *pdev)
{
	...
	wmt_pcm_controller_data[0].dma_cfg = dma_device_cfg_table[PCM1_TX_DMA_REQ];
	wmt_pcm_controller_data[1].dma_cfg = dma_device_cfg_table[PCM1_RX_DMA_REQ];
